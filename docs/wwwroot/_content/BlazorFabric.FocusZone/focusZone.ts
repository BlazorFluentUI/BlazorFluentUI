...
namespace BlazorFabricFocusZone {
    ...
    class FocusZoneInternal {
        ...
        */
        ...
        public updateFocusZone(props: FocusZoneProps) {
            ...
            if (this._root) {
                const windowElement = window.BlazorFabricBaseComponent.getWindow(this._root);
                let parentElement = window.BlazorFabricBaseComponent.getParent(this._root, ALLOW_VIRTUAL_ELEMENTS);

                while (parentElement && parentElement !== document.body && parentElement.nodeType === 1) {
                    if (window.BlazorFabricBaseComponent.isElementFocusZone(parentElement)) {
                        this._isInnerZone = true;
                        break;
                    }
                    parentElement = window.BlazorFabricBaseComponent.getParent(parentElement, ALLOW_VIRTUAL_ELEMENTS);
                }
                ...
                if (windowElement && outerZones.size === 1) {
                    _disposeGlobalKeyDownListener = window.BlazorFabricBaseComponent.on(windowElement, 'keydown', this._onKeyDownCapture, true);
                }
        ...
        private initialized() {
            const windowElement : Window = window.BlazorFabricBaseComponent.getWindow(this._root);
            let parentElement: HTMLElement = window.BlazorFabricBaseComponent.getParent(this._root, ALLOW_VIRTUAL_ELEMENTS);
       ...
       */
        ...
        private _onKeyDown = (ev: KeyboardEvent): void => {
            ...
            if (innerZoneKeystrokeTriggers && (innerZoneKeystrokeTriggers.indexOf(ev.keyCode) != -1) && this._isImmediateDescendantOfZone(ev.target as HTMLElement)) {
                ...
                } else if (window.BlazorFabricBaseComponent.isElementFocusSubZone(ev.target as HTMLElement)) {
                    if (
                        !this.focusElement(window.BlazorFabricBaseComponent.getNextElement(
                ...
                switch (ev.which) {
                    case window.BlazorFabricBaseComponent.KeyCodes.space:
                        if (this._tryInvokeClickForFocusable(ev.target as HTMLElement)) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.left:
                        if (direction !== FocusZoneDirection.vertical && this._moveFocusLeft()) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.right:
                        if (direction !== FocusZoneDirection.vertical && this._moveFocusRight()) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.up:
                        if (direction !== FocusZoneDirection.horizontal && this._moveFocusUp()) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.down:
                        if (direction !== FocusZoneDirection.horizontal && this._moveFocusDown()) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.tab:
                        if (
                            ...
                            if (
                                ...
                                const tabWithDirection = window.BlazorFabricBaseComponent.getRTL() ? !ev.shiftKey : ev.shiftKey;
                    ...
                    case window.BlazorFabricBaseComponent.KeyCodes.home:
                        ...
                        if (this._root && firstChild && this.focusElement(window.BlazorFabricBaseComponent.getNextElement(this._root, firstChild, true) as HTMLElement)) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.end:
                        ...
                        if (this._root && this.focusElement(window.BlazorFabricBaseComponent.getPreviousElement(this._root, lastChild, true, true, true) as HTMLElement)) {
                            break;
                        }
                        return;

                    case window.BlazorFabricBaseComponent.KeyCodes.enter:
                        if (this._tryInvokeClickForFocusable(ev.target as HTMLElement)) {
                            break;
                        }
                        return;
        ...
        private _onFocus = (ev: FocusEvent): void => {
            ...
            if (isImmediateDescendant) {
                ...
                while (parentElement && parentElement !== this._root) {
                    if (window.BlazorFabricBaseComponent.isElementTabbable(parentElement) && this._isImmediateDescendantOfZone(parentElement)) {
                        newActiveElement = parentElement;
                        break;
                    }
                    parentElement = window.BlazorFabricBaseComponent.getParent(parentElement, ALLOW_VIRTUAL_ELEMENTS) as HTMLElement;
                }
            }
        ...
        private _onKeyDownCapture = (ev: KeyboardEvent): void => {
            ...
            if (ev.which === window.BlazorFabricBaseComponent.KeyCodes.tab) {
        ...
        private _onMouseDown = (ev: MouseEvent): void => {
            ...
            while (target && target !== this._root) {
                path.push(target);
                target = window.BlazorFabricBaseComponent.getParent(target, ALLOW_VIRTUAL_ELEMENTS) as HTMLElement;
            }

            while (path.length) {
                target = path.pop() as HTMLElement;

                if (target && window.BlazorFabricBaseComponent.isElementTabbable(target)) {
                    this._setActiveElement(target, true);
                }

                if (window.BlazorFabricBaseComponent.isElementFocusZone(target)) {
        ...
        public focus(forceIntoFirstElement: boolean = false): boolean {
            if (this._root) {
                if (!forceIntoFirstElement && this._root.getAttribute(IS_FOCUSABLE_ATTRIBUTE) === 'true' && this._isInnerZone) {
                    ...
                    this._activeElement &&
                    window.BlazorFabricBaseComponent.elementContains(this._root, this._activeElement) &&
                    window.BlazorFabricBaseComponent.isElementTabbable(this._activeElement)
                ) {
                    ...
                    return this.focusElement(window.BlazorFabricBaseComponent.getNextElement(this._root, firstChild, true) as HTMLElement);
                }
            }
            return false;
        }
        ...
        private _updateTabIndexes(element?: HTMLElement) {
            if (!element && this._root) {
                ...
                element = this._root;
                if (this._activeElement && !window.BlazorFabricBaseComponent.elementContains(element, this._activeElement)) {
            ...
            if (this._activeElement && !window.BlazorFabricBaseComponent.isElementTabbable(this._activeElement)) {
                this._activeElement = null;
            }
            ...
            for (let childIndex = 0; childNodes && childIndex < childNodes.length; childIndex++) {
                ...
                if (!window.BlazorFabricBaseComponent.isElementFocusZone(child)) {
                    // If the item is explicitly set to not be focusable then TABINDEX needs to be set to -1.
                    if (child.getAttribute && child.getAttribute(IS_FOCUSABLE_ATTRIBUTE) === 'false') {
                        child.setAttribute(TABINDEX, '-1');
                    }

                    if (window.BlazorFabricBaseComponent.isElementTabbable(child)) {
        ...
        private _getOwnerZone(element?: HTMLElement): HTMLElement | null {
            let parentElement = window.BlazorFabricBaseComponent.getParent(element as HTMLElement, ALLOW_VIRTUAL_ELEMENTS);
        ...
        private _setActiveElement(element: HTMLElement, forceAlignment?: boolean): void {
            ...
            if (previousActiveElement) {
                if (window.BlazorFabricBaseComponent.isElementFocusZone(previousActiveElement)) {
   ...
   */
        private _getFirstInnerZone(rootElement?: HTMLElement | null): FocusZoneInternal | null {
            ...
            if (window.BlazorFabricBaseComponent.isElementFocusZone(rootElement)) {
       ...
       */
        private _tryInvokeClickForFocusable(target: HTMLElement): boolean {
            ...
            do {
                ...
                if (
                    ...
                    window.BlazorFabricBaseComponent.raiseClick(target);
                    return true;
                }
        ...
        */
        private _portalContainsElement(element: HTMLElement): boolean {
            ...
            //return element && !!this._root && BlazorFabricBaseComponent portalContainsElement(element, this._root.current);
        }
        ...
        private _shouldWrapFocus(element: HTMLElement, noWrapDataAttribute: 'data-no-vertical-wrap' | 'data-no-horizontal-wrap'): boolean {
            return !!this._focusZoneProps.checkForNoWrap ? window.BlazorFabricBaseComponent.shouldWrapFocus(element, noWrapDataAttribute) : true;
        }
        ...
        private _moveFocus(
            ...
            do {
                element = (isForward ? window.BlazorFabricBaseComponent.getNextElement(this._root, element) : window.BlazorFabricBaseComponent.getPreviousElement(this._root, element)) as HTMLElement;
            ...
            if (candidateElement && candidateElement !== this._activeElement) {
                ...
                if (isForward) {
                    return this.focusElement(window.BlazorFabricBaseComponent.getNextElement(
                        this._root,
                        this._root.firstElementChild as HTMLElement,
                        true
                    ) as HTMLElement);
                } else {
                    return this.focusElement(window.BlazorFabricBaseComponent.getPreviousElement(
                        this._root,
        ...
        private _moveFocusLeft(): boolean {
            ...
            if (
                this._moveFocus(
                    window.BlazorFabricBaseComponent.getRTL(),
                    (activeRect: ClientRect, targetRect: ClientRect) => {
                        let distance = -1;
                        let topBottomComparison;

                        if (window.BlazorFabricBaseComponent.getRTL()) {
